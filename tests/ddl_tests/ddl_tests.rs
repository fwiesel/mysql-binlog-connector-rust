...
mod test {
    ...
    fn run_sql_and_check_binlog(prepare_sqls: &[&str], test_sqls: &[&str], with_quotes: bool) {
        ...
        // MariaDB interleaves a short `# Dummy event` Query_event before each
        // real DDL when GTID replication is enabled; consumers see them but
        // they don't correspond to anything we executed, so drop them here.
        let ddl_queries: Vec<&str> = runner
            .query_events
            .iter()
            .map(|q| q.query.as_str())
            .filter(|q| !q.trim_start().starts_with('#'))
            .collect();
